...
if (isset($ftp_username))
{
 ...
 if (!$save_backup_zip_file_to_server) unlink(LOCATION."../../../../db_backup_restore/".$backup_file_name);
}

if(!session_id()) session_start();
if(isset($_SESSION['pmab_mysql_errors'])) $errors.=$_SESSION['pmab_mysql_errors'];

//if u want attachement file - currently no!
...
/*if ($send_email_report)
{
 ...
 if ($save_backup_zip_file_to_server) $msg_local_backup="\nthe backup zip file has been saved to the same server: (Folder: db_backup_restore) \n";
 if ($errors=="") $errors="None captured."; 
 mail($report_emailaddress,
                                  "REPORT on recent backup using QRM AutoBackup ($backup_file_name)",
...
//send me updates on successful backups
$IssueDef = "REPORT on recent backup using AutoBackup ($backup_file_name)\n
ERRORS: $errors \nSAVE or DELETE THIS MESSAGE - no backup is attached. $msg_ftp_backup $msg_local_backup";
$IssueSubj = "QRM Backup Status";

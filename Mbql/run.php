...
********************************************************************************************/
//php needs more execution time and memory, handles timeout
...
//run clean up DB
//delete all previous day processed messages
//previous date
$PrvDt = date('Y-m-d',strtotime("-1 days"));
//cleanup files log
$ClnFils = Delcln("file","fl_resubflg",1,"fl_pdate",$PrvDt);
//previous date +2 to remove possibility of destabilizing on errors resubmit
$PrvDt2 = date('Y-m-d',strtotime("-5 days"));
$PrvDt3 = date('Y-m-d',strtotime("-2 days"));
$PrvDt4 = date('Y-m-d',strtotime("-14 days"));
//clean up statements resublog
$ClnStmt = Delcln2("resub","re_date",$PrvDt2);
//clean up statements errorlogs
$ClnErhnd = Delcln2("errhd","re_date",$PrvDt);//after one day clear log error handle
$ClnReslog = DelAll("reslog","re_date",$PrvDt3);//after two days clear log error logger
...
$from_emailaddress = "SBK_Policycentral@stanbic.com";// your email address to show who the email is from (should be different $to_emailaddress)
$report_emailaddress = "wanjohig@stanbic.com";//address to send reports, not the backup just details on last backups (can be same as above)
$to_emailaddress = "wanjohig@stanbic.com"; // your email address to send backup files to

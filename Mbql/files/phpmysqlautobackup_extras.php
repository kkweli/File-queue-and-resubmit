...
{
 $fp = fopen(LOCATION."../../../../db_backup_restore/".$backup_file_name, "w");
 fwrite($fp, $gzdata);
 fclose($fp);
...
function delete_old_backups()
{
    ...
    $suffix = ".sql.gz";
    $dir = LOCATION."../../../../db_backup_restore/";
...
class transfer_backup
{
      ...
      public function transfer_data($ftp_username,$ftp_password,$ftp_server,$ftp_path,$filename)
      {
       ...
       {
        $file=LOCATION."../../../../db_backup_restore/".$filename;
        $fp = fopen($file, "r");
        $ch = curl_init();
...
class record
{
      public function save($date, $bytes, $lines, $logdt)
      {
       $dbc = dbc::instance();
       ...
       if(count($rows)<1)
       {
        $q="CREATE TABLE IF NOT EXISTS `phpmysqlautobackup_log` (
            ...
            `logdate` var(20) NOT NULL,
             PRIMARY KEY (`date`) )";
        $result = $dbc->prepare($q);
        $result = $dbc->execute($result);
       }
       $query="INSERT INTO `phpmysqlautobackup_log` (`date`, `bytes`, `lines`, `logdate`)
                 VALUES ('$date', '$bytes', '$lines', '$logdt')";
